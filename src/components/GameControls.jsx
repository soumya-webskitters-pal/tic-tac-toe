...
export default function GameControls({ game, onStart }) {
  ...
  useEffect(() => {
    ...
    if (!room || joinedFromLink.current) return
    joinedFromLink.current = true; setMode('online'); game.joinOnline(room)
  }, [])
  return <div className="controls">
    <button className={`play-friends-cta ${mode==='online'?'active':''}`} onClick={()=>setMode('online')}><span className="friends-icon">♟♟</span><span><b>PLAY WITH FRIENDS</b><small>Challenge someone online</small></span><i>→</i></button>
    <div className="control-section"><label>GAME MODE</label><div className="segmented"><button className={mode==='ai'?'selected':''} onClick={()=>setMode('ai')}><Icon>✦</Icon> VS AI</button><button className={mode==='pvp'?'selected':''} onClick={()=>setMode('pvp')}><Icon>♟</Icon> 2 PLAYER (OFFLINE)</button></div></div>

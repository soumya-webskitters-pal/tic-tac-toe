...
export default function OnlineLobby({ game, onReady }) {
  ...
  const hosting = game.onlineStatus === 'creating' || game.onlineStatus === 'waiting'
  ...
  return <div className="online-lobby"><button className="host-room" onClick={game.hostOnline} disabled={Boolean(code.trim())||hosting}>CREATE A ROOM</button>{hosting && game.roomCode && <div className="room-code"><span>{game.roomCode}</span><button onClick={() => navigator.clipboard?.writeText(game.roomCode)}>COPY</button><button className="share-room" onClick={shareRoom} aria-label="Share game link" title="Share game link">↗</button></div>}{game.onlineStatus==='waiting'&&<p>Share the code or link with your friend. Waiting for them…</p>}{!hosting&&<div className="join-row"><input value={code} onChange={e=>setCode(e.target.value)} placeholder="Enter room code"/><button onClick={()=>game.joinOnline(code)} disabled={!code.trim()}>JOIN</button></div>}{game.onlineStatus==='joining'&&<p>Connecting…</p>}{game.onlineStatus==='error'&&<p className="online-error">Could not connect. Check the code and retry.</p>}</div>
}

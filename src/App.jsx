...
export default function App() {
  ...
  return <main style={{ '--accent':theme.value, '--accent-rgb':theme.rgb }}><div className="aurora a1"/><div className="aurora a2"/><div className="noise"/><Header themes={THEMES} theme={theme} onThemeChange={setTheme} profile={profile} onProfile={()=>setProfileOpen(true)}/><Hero/><section className="game-shell"><GameArena game={game} onNewMatch={() => setSetupOpen(true)} onPlayFriends={openFriends} onExitFriends={exitFriends}/></section><Footer onPrivacy={()=>setLegalPage('privacy')} onTerms={()=>setLegalPage('terms')}/>{setupOpen && <MatchSetupModal game={game} profile={profile} onClose={closeSetup} onStart={() => setSetupOpen(false)}/>} {!setupOpen && showResult && <ResultModal game={game} onNewMatch={() => setSetupOpen(true)}/>} {profileOpen&&<ProfileModal profile={profile} history={game.gameHistory} onSave={setProfile} onClose={()=>setProfileOpen(false)}/>} {legalPage&&<LegalModal page={legalPage} onClose={()=>setLegalPage(null)}/>}<AnalyticsConsent ageGroup={['under_13','13_17','18_24','25_34','35_plus'][profile.ageGroup??2]} onConsent={setAnalyticsReady}/></main>
}
